  ...
  return (
    <div className="searchHistory">
      {historyLoaded.map((item, i) => (
        <span
          key={item.id}
          onMouseEnter={() => setHistorySelected(i)}
          className={historySelected == i ? "active" : undefined}
        >
          <a
            className="searchHistory__item__content"
            onMouseDown={() => fetch(item.document.search)}
          >
            <span>🔍</span>
            <span>{item.document.search}</span>
          </a>
          <a
            className="searchHistory__item__delete"
            onMouseDown={(e) => {
              deleteFromHistory(i);
              e.preventDefault();
            }}
